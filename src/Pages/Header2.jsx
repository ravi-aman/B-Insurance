...
function Header() {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            // Slide down on scroll up, slide up on scroll down
            if (currentScroll < lastScrollTop) {
                gsap.to(header, { y: 0, duration: 0.5, ease: "power3.out" });
            } else {
                gsap.to(header, { y: -header.offsetHeight, duration: 0.5, ease: "power3.out" });
            }
    ...
    return (
        <header className="bg-white shadow-sm fixed top-0 w-full z-50" ref={headerRef}>
